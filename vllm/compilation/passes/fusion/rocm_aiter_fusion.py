...
class MLADualRMSPerGroupQuantPattern(
    VllmPatternReplacement[
        ...,
        tuple[
            torch.Tensor,
    ...
    """
    Fuse the MLA FP8 attention path -- q-latent RMSNorm + FP8 *per-group* quant
    plus kv-latent RMSNorm -- into AITER's ``fused_qk_rmsnorm_group_quant``.

    With a per-group FP8 ``q_b_proj`` (block-scaled, ``GroupShape(1, 128)``),
    the earlier ``RocmAiterRMSNormQuantFusionPass`` folds the q side into
    ``rocm_aiter_rmsnorm_fp8_group_quant`` and leaves the kv side a plain
    ``vllm_ir.rms_norm``. This pattern matches that asymmetric pair::
...
class MLADualRMSNormFusionPass(VllmFusionPatternMatcherPass):
    ...
    :class:`MLADualRMSPerTokenQuantPattern` (per-token quant) and
    :class:`MLADualRMSPerGroupQuantPattern` (per-group block-scaled quant),
    which fuse the q-latent RMSNorm + FP8 quant together with the kv-latent
    RMSNorm into a single AITER kernel call.
    """
    ...
    def __init__(self, config: VllmConfig) -> None:
        ...
        for epsilon in [1e-5, 1e-6]:
            ...
            self.register(MLADualRMSPerGroupQuantPattern(epsilon))
