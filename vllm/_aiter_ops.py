...
def _fused_mla_dual_rms_norm_per_group_quant_impl(
    ...
    """Fused MLA q/kv RMSNorm (+ FP8 per-group quant on q) via AITER.

    Backs the ``fused_mla_dual_rms_norm_per_group_quant`` custom op used by the
    MLA FP8 attention fusion when the q latent is quantized with *per-group*
    block scales (``(M, ceil(N/group_size))``). Only the *q* latent is FP8
    quantized (it feeds the FP8 ``q_b_proj`` GEMM); the *kv* latent is
    RMS-normed and consumed by attention as bf16.
    """
    from aiter.ops.fused_qk_rmsnorm_group_quant import (
